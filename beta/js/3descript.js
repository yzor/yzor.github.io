...
function description(QWE) { //генерация описания
  ...
  //+заменить→&nbsp;← на → ← вроде он-же /не было ; на конце, фикс
  ...
  $.when.apply($, deferreds).done(function () { //дождатся окончания ajax
          ...
          $.each(v, function (I) { // 2 перебор only закрашеных клеток
            ...
            if (isRap(i, v[I]) && !isRap(i, v[I + 1])) {
              ...
              spase = "<span class='star l'>*</span>" + //💩
                "</span>, повторять&nbsp;от&nbsp;<span class='star2 b'>*</span>&nbsp;до&nbsp;<span class='star2 a'>*</span>";
    ...
    $("#d").html(D); //выводим описание

    //добавляем комментарии в копируемый текст
    copyTxt = '<!--max:start--><!--more--><div id="d">' + D + '</div><!--max:end-->';
    copyTest();
  }); //end ajax
}
