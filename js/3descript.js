...
function descrip() { //генерация описания
  ...
  $.each(SVG.LIST.reverse(), function (i) { //перебор списка клеток
    ...
    if (num == 1) {
      if (line != 1) { //если первый номер в сторке, то выводим закрывающий тег(кроме 1 строки)
        SVG.D = SVG.D + "</ul>";
      }
    ...
    if ( //вторая звёздочка
      ...
      afterStar = "<li class='raport l'>*</li>" + //💩
        ...
        " повторять от <li class='raport l'>*</li> до <li class='raport l'>*</li>" + // после 2звёздочки, запятой и пробела "*, "
